...
%        temp_averagevelocity(k) = (peak(1) - minn(1))/((peak(2) - minn(2))/(sampling_rate/25));
    ...
    animal_stats1(j,1) = mean(temp_peak_time(find(temp_peak_magn <= 1.5))); % peak time
    ...
    animal_stats2(j,1) = mean(temp_peak_magn(find(temp_peak_magn <= 1.5))); % peak magnitude
    ...
    animal_stats3(j,1) = mean(temp_onset_time(intersect(find(~isnan(temp_onset_time)),find(temp_peak_magn <= 1.5)))); % onset time
    ...
    animal_stats4(j,1) = mean(temp_peak_time(find(temp_peak_magn <= 1.5))-temp_onset_time(find(temp_peak_magn <= 1.5))); % rise time
    ...
    animal_stats5(j,1) = mean(temp_onset_value(intersect(find(~isnan(temp_onset_value)),find(temp_peak_magn <= 1.5))));
...
%    animal_stats6(j,3) = size(temp_averagevelocity(find(temp_peak_magn <= 1.5)),1);
    animal_stats8(j,1) = mean(temp_1090time(find(temp_peak_magn <= 1.5)));
